  ...
  reportOn(profileRunner) {
    ...
    profileRunner.on(events.PROFILE_END, (profile, result) => {
      if (this.verbosity >= VERBOSITY.NORMAL) {
        ...
        if (fastest.length === 1) {
          console.info(chalk.green(`  ${fastest[0].func.description()} (${this.formatTime(fastest[0].averageTime)})\n`));
        } else {
          console.info(chalk.green(`  Draw between:\n${fastest.forEach((b) => `  ${b.func.description()} (${this.formatTime(b.averageTime)})`)}`));
        }
    ...
    profileRunner.on(events.TEST_END, (profile, func, result) => {
      switch(this.verbosity) {
        case VERBOSITY.NORMAL:
          if (this.memory) {
            console.info(`    ${this.formatTime(result.averageTime)} | ${this.formatMemory(result.memory.average)}\n`);
          } else {
        ...
        case VERBOSITY.VERBOSE:
          console.info(`    TIME:`);
          console.info(`      avg: ${this.formatTime(result.averageTime)}`);
          if (this.memory) {
            console.info(`    MEMORY:`);
