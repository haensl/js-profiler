  ...
  */
      ...
      profileRunner.on(events.PROFILE_END, (profile, result) => {
        ...
        if (this.verbosity >= VERBOSITY.NORMAL) {
          ...
          p.fastest = best.map((b) => ({
            ...
            average: this.formatTime(b.average),
            minimum: this.formatTime(b.minimum),
            maximum: this.formatTime(b.maximum)
          }));
        }
      ...
      profileRunner.on(events.TEST_END, (profile, func, result) => {
        if (this.verbosity >= VERBOSITY.NORMAL) {
          this.profiles[profile.name].push({
            ...
            average: this.formatTime(result.average),
            minimum: this.formatTime(result.minimum),
            maximum: this.formatTime(result.maximum)
          });
        }
      });
    });
  ...
  bestResults(results) {
    return results.testResults
      ...
      .filter((a, i, arr) => a.average === arr[0].average);
  }
    ...
    switch(this.unit) {
      case UNITS.AUTO:
        if (t.toFixed(this.precision) === 0) {
          return `${UNITS.microToMilli(t).toFixed(this.precision)}ms`;
        }

        return `${t.toFixed(this.precision)}µs`;
      case UNITS.MICROSECONDS:
        return `${t.toFixed(this.precision)}μs`;
      default:
        return `${UNITS.microToMilli(t).toFixed(this.precision)}ms`;
    }
  }
