  ...
  */
  ...
  formatProfileResult(profile, result) {
    ...
    if (this.options.verbosity >= VERBOSITY.NORMAL) {
      ...
      p.keywords = profile.keywords;
      p.functions = this.profiles[profile.name];
  ...
  formatTestResult(testResult) {
    const r = {
      ...
      keywords: testResult.func.keywords,
      codeSample: testResult.func.codeSample,
      code: `${testResult.func.f}`,
      time: {
        average: this.formatMillis(testResult.time.average)
      }
    };

    if (this.options.verbosity === VERBOSITY.VERBOSE) {
      r.time = Object.assign(r.time, {
        minimum: this.formatMillis(testResult.time.minimum),
        maximum: this.formatMillis(testResult.time.maximum)
      });
    }
  ...
  formatMillis(millis) {
    switch(this.options.units.time) {
      case UNITS.AUTO:
        if (millis < 1) {
          return `${UNITS.milliToMicro(millis).toFixed(this.options.precision.time)}µs`;
        }

        return `${millis.toFixed(this.options.precision.time)}ms`;
